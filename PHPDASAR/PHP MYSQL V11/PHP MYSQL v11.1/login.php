...
// cek cookie & username
if ($USERID = hash('gost-crypto', $row['USERID']) && $USERKEY === hash('sha256', $row['username'])) {
	$_SESSION['login'] = true;
}
...
// cek tombol login is checked
	if ( isset($_POST['login']) ) {
		...
		if (isset($_POST['remember'])) {
			...
			setcookie('USERID', hash('gost-crypto', $row['USERID']), time() + 60);
			setcookie('USERKEY', hash('sha256', $row['username']), time() + 60);
